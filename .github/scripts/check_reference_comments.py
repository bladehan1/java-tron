...
arrays, and the genesis.block exemption.
...
def collect_keys(path, list_all=False):
    ...
    seen_rows : list of (line_no, key, status)
        ...
        *list_all* is True (``--list`` flag); always empty otherwise.
        status is one of: "commented" | "exempt" | "dedup" | "missing".
    """
    ...
    #                          anonymous braces/brackets.  Used to detect the
    #                          genesis.block exemption zone.
    ...
    for index, raw in enumerate(lines):
        ...
        if match:
            ...
            # --- Exemption 1: genesis.block zone ---
            # Any key whose path is inside a genesis.block frame is skipped
            # entirely; private-chain operators freely customise genesis data.
            exempt = in_genesis_block(stack, key)

            # --- Exemption 2: array deduplication ---
