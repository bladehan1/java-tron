...
/**
 * Unit tests for the 0x1b batch independent ML-DSA-44 verify precompile.
...
public class BatchValidateMlDsa44Test {
  ...
  private static final DataWord ADDR_0X1B = new DataWord(
  ...
  public void switchOff_returnsNull() {
    VMConfig.initAllowMlDsa44(0L);
    Assert.assertNull(PrecompiledContracts.getContractForAddress(ADDR_0X1B));
  }

  @Test
  public void switchOn_returnsContract() {
    PrecompiledContract pc = PrecompiledContracts.getContractForAddress(ADDR_0X1B);
  ...
  private Pair<Boolean, byte[]> run(byte[] hash, List<String> sigs,
    ...
    logger.info("0x1b bitmap: {}", Hex.toHexString(ret.getRight()));
    return ret;
  }
