...
public class MLDSA44KatTest {
  ...
  public void allVectorsDeriveExpectedPublicAndPrivateKey() {
    for (KatVector v : VECTORS) {
      ...
      assertEquals(v.label + ": pk length",
          MLDSA44.PUBLIC_KEY_LENGTH, k.getPublicKey().length);
      assertEquals(v.label + ": sk length",
          MLDSA44.PRIVATE_KEY_LENGTH, k.getPrivateKey().length);
  ...
  public void allVectorsDeriveExpectedAddress() {
    for (KatVector v : VECTORS) {
      ...
      byte[] viaRegistry =
          PQSchemeRegistry.computeAddress(PQScheme.ML_DSA_44, k.getPublicKey());
  ...
  public void signaturesFromKatKeysVerifyUnderTheirOwnPublicKey() {
    byte[][] messages = {
        new byte[0],
        "x".getBytes(),
        "tron-ml-dsa-kat-message".getBytes(),
        new byte[1024],
    };
    for (KatVector v : VECTORS) {
      MLDSA44 k = new MLDSA44(v.seed);
